...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_department'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO department (departmentName, description, headOfDepartment) VALUES (?, ?, ?)");
        $stmt->bind_param("ssi", $departmentName, $description, $headOfDepartment);
        $stmt->execute();
        $stmt->close();
...
$query = "
    SELECT d.departmentId, d.departmentName, d.description, e.name
    FROM department d
    JOIN employee e ON d.headOfDepartment = e.employeeId
";
$result = $conn->query($query);

$departments = '';
while ($row = $result->fetch_assoc()) {
    $departments .= '<div class="flex justify-between items-center mb-4">
        <div>
            <p class="text-lg font-bold">' . $row['departmentName'] . '</p>
        ...
        <form method="POST" class="ml-4">
            ...
            <button type="submit" name="delete_department" class="bg-red-500 text-white px-4 py-2 rounded">Delete</button>
        </form>
    </div>';
}
$conn->close();
    ...
    <div class="bg-white shadow-md rounded-lg p-4 mb-6">
        ...
        <form method="POST" class="grid grid-cols-1 gap-4">
            <div>
                ...
                <input type="text" name="departmentName" id="departmentName" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            </div>
            <div>
            ...
            <div>
                <label for="headOfDepartment" class="block text-sm font-medium text-gray-700">Head of Department</label>
                <input type="number" name="headOfDepartment" id="headOfDepartment" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            </div>
            <button type="submit" name="add_department" class="bg-indigo-500 text-white px-4 py-2 rounded">Add Department</button>
        </form>
    </div>
